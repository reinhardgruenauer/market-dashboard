...
def _fetch_news():
    all_news = {}
    symbols_names = {s["symbol"]: s["name"] for s in SP500_TOP10 + NQ100_TOP10}
    ...
    try:
        with open(CALENDAR_CACHE_FILE, "w", encoding="utf-8") as f:
            json.dump({"date": str(datetime.date.today()),
                       ...
                       "raw_data": raw_data, "events": events},
...
def _parse_ff_events(raw_data):
    ...
    country_map = {"USD":"USA","EUR":"Eurozone","GBP":"Großbritannien",
                   "JPY":"Japan","CAD":"Kanada","AUD":"Australien",
                   "NZD":"Neuseeland","CHF":"Schweiz","CNY":"China"}
    for event in raw_data:
        if event.get("impact","") != "High": continue
        try:
            event_date = datetime.datetime.fromisoformat(event.get("date","")).date()
        except Exception:
            try: event_date = datetime.datetime.strptime(event.get("date","")[:10],"%Y-%m-%d").date()
            except Exception: continue
        if event_date != today: continue
        try:
            edt = datetime.datetime.fromisoformat(event.get("date",""))
            if edt.tzinfo is None:
                edt = edt.replace(tzinfo=ZoneInfo("America/New_York"))
            event_time = edt.astimezone(TZ_VIENNA).strftime("%H:%M")
        except Exception:
            event_time = ""
        def _de(v):
            if not v or not isinstance(v,str): return v or ""
            return v.replace(".",",") if "." in v else v
        events.append({"time": event_time,
                        "country": country_map.get(event.get("country",""), event.get("country","")),
                        "currency": event.get("country",""),
                        "title": _translate_event_title(event.get("title","")),
                        "impact": "high",
                        "forecast": _de(event.get("forecast","")),
                        "previous": _de(event.get("previous","")),
                        "actual":   _de(event.get("actual",""))})
...
def api_data():
    ...
    sp500_stocks = [{**s, **stock_data.get(s["symbol"],{}),
                     "news": news_data.get(s["symbol"],[])} for s in SP500_TOP10]
    nq100_stocks = [{**s, **stock_data.get(s["symbol"],{}),
                     "news": news_data.get(s["symbol"],[])} for s in NQ100_TOP10]

    return jsonify({
        "timestamp": datetime.datetime.now(TZ_VIENNA).strftime("%Y-%m-%d %H:%M:%S") + " MEZ",
        "futures":  {"ES": stock_data.get("ES=F",{}), "NQ": stock_data.get("NQ=F",{})},
        "sp500":    {"stocks": sp500_stocks, "probability": _calc_probability(SP500_TOP10, stock_data)},
        "nq100":    {"stocks": nq100_stocks, "probability": _calc_probability(NQ100_TOP10, stock_data)},
